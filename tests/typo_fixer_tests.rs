			...
			if cli_config_path.is_file() {
				...
				if let Some(model_path) = v["model_info"]["path"].as_str() {
					...
					if mdir.is_dir() {
						...
						eprintln!("[typo-fixer] CLI model path not found, and no fixtures; skipping.");
						return Ok(None);
					}
				} else {
					(default_model_dir, default_config_path)
...
fn test_step5_prefill_lm_head_matches_fixture() -> anyhow::Result<()> {
	...
	let causal = model
		.config()
		.create_causal_mask_with_mode_detection(0, model.config().context_length(), true)?;
	use candle_core::Tensor;
	let current_pos_t = Tensor::from_vec(vec![(context_pos - 1) as i64], (1,), &model.config().device)?;
	model.initialize_states()?;
	let prefill_last = model.run_ffn_prefill_with_inputs(&embeddings, &position_ids, &causal, &current_pos_t)?;
	...
	let expected_token_text = v5["data"]["top_predictions"]["tokens"][0]
		.as_str()
		.unwrap_or(".");
	let decoded = model.tokenizer().decode(&[best_idx as u32], false).unwrap_or_default();
	if decoded != expected_token_text {
		eprintln!(
			"[step5-prefill-lm] mismatch: got '{}' (id={}), expected '{}'",
			decoded, best_idx, expected_token_text
		);
...
fn test_step5_infer_top1_matches_fixture_using_prefill_output() -> anyhow::Result<()> {
	...
	// 4) current_pos [1]
	use candle_core::Tensor;
	let current_pos_t = Tensor::from_vec(
		vec![(context_pos - 1) as i64],
		(1,),
		&model.config().device,
	)?;

	model.initialize_states()?; // ensure unified state exists
	let prefill_last = model.run_ffn_prefill_with_inputs(
		&embeddings,
		&position_ids,
		&causal,
		&current_pos_t,
	...
	if decoded != expected_token_text {
		eprintln!(
			"[step5-prefill-out] mismatch: got '{}' (id={}), expected '{}'",
			decoded, best_idx, expected_token_text
		);
...
fn test_step2_causal_mask_matches_fixture_sample() -> anyhow::Result<()> {
	...
	let row0_expected: Vec<f32> = fx_rows[0]
		...
		.map(|x| {
			if let Some(f) = x.as_f64() {
				let f = f as f32;
				if f < -1.0e30f32 { f32::NEG_INFINITY } else { f }
			} else {
				f32::NEG_INFINITY
			}
		})
	...
	if row0_actual.len() > 16 { row0_actual.truncate(16); }
	...
	if dims[2] >= 12 {
		...
		if row11_actual.len() > 16 { row11_actual.truncate(16); }
...
fn test_step5_infer_top1_matches_fixture() -> anyhow::Result<()> {
	...
	eprintln!("[step5] RUST infer hidden_states shape: {:?}", hidden_states.dims());
	...
	eprintln!("[step5] infer input hidden_states shape: {:?}", hidden_states.dims());
	if hidden_states.dims().len() == 2 {
		...
		if let Ok(hs_slice) = hidden_states.narrow(2, 0, 16.min(hidden_states.dims()[2])) {
			if let Ok(values) = hs_slice.to_vec3::<f32>() {
				eprintln!("[step5] infer input hidden_states[0,0,0..16]: {:?}", &values[0][0]);
			}
		}
	}
	
	let infer_out = model.run_ffn_infer_with_inputs(&hidden_states, &position_ids, &causal, &current_pos)?;
	
	// Debug: Print infer output hidden states 
	eprintln!("[step5] infer output hidden_states shape: {:?}", infer_out.dims());
	if infer_out.dims().len() == 3 && infer_out.dims()[2] >= 16 {
		if let Ok(out_slice) = infer_out.narrow(2, 0, 16.min(infer_out.dims()[2])) {
			if let Ok(values) = out_slice.to_vec3::<f32>() {
				eprintln!("[step5] infer output hidden_states[0,0,0..16]: {:?}", &values[0][0]);
			}
		}
	} else {
		eprintln!("[step5] infer output: unexpected shape or insufficient dimensions for debug display");
	}
	...
	if let Some(fixture_infer_output) = v5["data"]["infer_output_hidden_states"].as_array() {
		if let Some(fixture_batch) = fixture_infer_output[0].as_array() {
			if let Some(fixture_seq) = fixture_batch[0].as_array() {
				let fixture_values: Vec<f32> = fixture_seq.iter()
					.take(16)
				...
				if infer_out.dims().len() == 3 && infer_out.dims()[2] >= 16 {
					if let Ok(rust_slice) = infer_out.narrow(2, 0, 16.min(infer_out.dims()[2])) {
						if let Ok(rust_values) = rust_slice.to_vec3::<f32>() {
							let rust_first_16 = &rust_values[0][0];
							eprintln!("[step5] FIXTURE infer output[0,0,0..16]: {:?}", &fixture_values);
							eprintln!("[step5] RUST    infer output[0,0,0..16]: {:?}", rust_first_16);
							
							// Check if they match within tolerance
							let matches = fixture_values.iter().zip(rust_first_16.iter())
								.all(|(f, r)| (f - r).abs() < 0.1);
	...
	eprintln!("[step5] lm_head chunk sizes = {:?} (sum = {}), model vocab_size = {}", chunk_sizes, total_vocab, model.config().vocab_size());
	...
	// Debug: show top-5 predictions (id, text, score)
	{
		...
		for (i, (idx, sc)) in top5.iter().enumerate() {
			let txt = model.tokenizer().decode(&[*idx as u32], false).unwrap_or_default();
	...
	for (rank, (tid, sc)) in indexed.iter().take(top_k).enumerate() {
		let txt = model.tokenizer().decode(&[*tid as u32], false).unwrap_or_default();
	...
	if decoded != expected_token_text {
		...
		if let Some(arr) = v5["data"]["top_predictions"]["tokens"].as_array() {
			let fx_tokens: Vec<String> = arr.iter().filter_map(|v| v.as_str().map(|s| s.to_string())).collect();

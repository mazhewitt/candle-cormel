...
fn test_embeddings_model_direct() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Simple input shape: {:?}, dtype: {:?}", simple_input.dims(), simple_input.dtype());
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        let max_abs_1 = embed_values_1.iter().map(|x| x.abs()).fold(0.0f32, f32::max);
        let max_abs_0 = embed_values_0.iter().map(|x| x.abs()).fold(0.0f32, f32::max);
        let max_abs_2 = embed_values_2.iter().map(|x| x.abs()).fold(0.0f32, f32::max);

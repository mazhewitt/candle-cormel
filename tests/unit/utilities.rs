...
use candle_coreml::utils::{mask, multi_component, sampling};
...
mod mask_tests {
    ...
    fn test_create_causal_mask_single_token() {
        ...
        assert_eq!(mask.to_vec2::<f32>().unwrap()[0][0], 0.0);
    }
    ...
    fn test_create_causal_mask_large() {
        ...
        for (i, row) in mask_data.iter().enumerate().take(seq_len) {
            for (j, &value) in row.iter().enumerate().take(seq_len) {
                if j > i {
                    assert_eq!(
                        value,
                        f32::NEG_INFINITY,
    ...
    fn test_create_position_mask_basic() {
        ...
        let mask_data = mask.to_vec2::<f32>().unwrap()[0].clone();
    ...
    fn test_create_position_mask_edge_cases() {
        ...
        let data = mask.to_vec2::<f32>().unwrap()[0].clone();
    ...
    fn test_create_rank4_position_mask() {
        ...
        let mask_data = mask.flatten_all().unwrap().to_vec1::<f32>().unwrap();
...
mod sampling_tests {
    ...
    fn test_greedy_sample() {
        ...
        let logits_data = vec![1.0f32, 2.0f32, 5.0f32, 0.5f32];
    ...
    fn test_greedy_sample_ties() {
        ...
        let logits_data = vec![3.0f32, 1.0f32, 3.0f32, 2.0f32];
        let logits = Tensor::from_vec(logits_data, (4,), &device).unwrap();

        let sampled_token = sampling::greedy_sample(&logits).unwrap();
        // Function returns last occurrence of max value due to max_by implementation
    ...
    fn test_sample_with_temperature_deterministic_high_temp() {
        ...
        let logits_data = vec![-10.0f32, -10.0f32, 10.0f32, -10.0f32];
        let logits = Tensor::from_vec(logits_data, (4,), &device).unwrap();
    ...
    fn test_sample_top_k_greedy() {
        let device = Device::Cpu;
        let logits_data = vec![1.0f32, 4.0f32, 2.0f32, 3.0f32];
        let logits = Tensor::from_vec(logits_data, (4,), &device).unwrap();
...
mod multi_component_tests {
    ...
    fn test_component_config_builder_ffn_with_mask() {
        ...
        assert_eq!(
            config.input_names,
            vec!["hidden_states".to_string(), "causal_mask".to_string()]
        );
    ...
    fn test_combine_chunked_logits_success() {
        ...
        let chunk1 = Tensor::from_vec(vec![1.0f32, 2.0f32], (1, 2), &device).unwrap();
        let chunk2 = Tensor::from_vec(vec![3.0f32, 4.0f32], (1, 2), &device).unwrap();
        let chunk3 = Tensor::from_vec(vec![5.0f32, 6.0f32], (1, 2), &device).unwrap();
        ...
        let combined_data = combined.to_vec2::<f32>().unwrap()[0].clone();
        assert_eq!(
            combined_data,
    ...
    fn test_combine_chunked_logits_single_chunk() {
        ...
        let chunk1 = Tensor::from_vec(vec![10.0f32, 20.0f32, 30.0f32], (1, 3), &device).unwrap();
        outputs.insert("logits1".to_string(), chunk1);
        ...
        let combined_data = combined.to_vec2::<f32>().unwrap()[0].clone();
        assert_eq!(combined_data, vec![10.0f32, 20.0f32, 30.0f32]);
    }
...
fn test_tensors_approx_equal_helper() {
    ...
    let tensor1 = Tensor::from_vec(vec![1.0f32, 2.0f32, 3.0f32], (3,), &device).unwrap();
    let tensor2 = Tensor::from_vec(vec![1.001f32, 1.999f32, 3.001f32], (3,), &device).unwrap();
    let tensor3 = Tensor::from_vec(vec![1.0f32, 2.0f32], (2,), &device).unwrap(); // Different shape

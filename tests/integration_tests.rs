...
use candle_coreml::{download_model, Config, CoreMLModel};
...
fn test_openelm_mlstate_baseline() {
    ...
    let tensor1 =
        Tensor::from_vec(padded_seq1, (1, 128), &device).expect("Should create first tensor");
    let tensor2 =
        Tensor::from_vec(padded_seq2, (1, 128), &device).expect("Should create second tensor");

    println!("📝 Testing predict_with_state() on full sequences");

    // Test predict_with_state with first state
    let output1 = model
        .predict_with_state(&[&tensor1], &mut state1)
    ...
    let output2 = model
        .predict_with_state(&[&tensor2], &mut state2)
    ...
    println!(
        "  Max difference: {:.6}, Significant differences: {}",
        max_diff, significant_diffs
    );

    // Results should be very similar (MLState might not affect this particular model much)
    assert!(
        max_diff < 2.0,
        "Stateful and stateless should be reasonably similar, max diff: {:.6}",
        max_diff
    );
    ...
    let output1_reused = model
        .predict_with_state(&[&tensor2], &mut state1)
        .expect("State reuse should work");

    assert_eq!(
        output1_reused.dims(),
        &[1, 128, 32000],
        "Reused state output should have correct shape"
    );
    ...
    assert!(
        top_score > -10.0,
        "Should produce reasonable top prediction"
    );
...
fn test_mistral_context_sensitivity() {
    ...
    let model =
        CoreMLModel::load_from_file(&model_path, &config).expect("Should load Mistral model");
    ...
    let input_token = Tensor::from_vec(vec![1i64], (1, 1), &device).expect("Create input tensor");

    let mask_pos0 = create_causal_mask(0);
    let output_pos0 = model
        .predict_with_state(&[&input_token, &mask_pos0], &mut state_pos0)
    ...
    let tokens_sequence = [1i64, 50i64, 1i64];
    let mut accumulated_logits = Vec::new();

    for (pos, &token) in tokens_sequence.iter().enumerate() {
        let token_tensor =
            Tensor::from_vec(vec![token], (1, 1), &device).expect("Create token tensor");

        let mask = create_causal_mask(pos);
        let output = model
            .predict_with_state(&[&token_tensor, &mask], &mut state_pos2)
    ...
    for (&logit0, &logit2) in logits_pos0.iter().zip(logits_pos2.iter()) {
    ...
    println!(
        "  Position 0 top token: {} (score: {:.3})",
        pos0_top_token, top_pos0[0].1
    );
    println!(
        "  Position 2 top token: {} (score: {:.3})",
        pos2_top_token, top_pos2[0].1
    );
    ...
    assert!(
        max_diff > 0.01,
        "Context should affect predictions (max diff: {:.6})",
        max_diff
    );
    assert!(
        significant_diffs > 100,
        "Many logits should change with context (changed: {})",
        significant_diffs
    );
    ...
    for (name, sequence) in test_sequences {
        ...
        for (pos, &token) in sequence.iter().enumerate() {
            ...
            let mask = create_causal_mask(pos);
            let output = model
                .predict_with_state(&[&token_tensor, &mask], &mut state)
    ...
    println!(
        "  Sequence A [1,10,20] → Token {} (score: {:.3})",
        seq_a_top, seq_a_score
    );
    println!(
        "  Sequence B [1,30,40] → Token {} (score: {:.3})",
        seq_b_top, seq_b_score
    );
    ...
    if seq_a_top != seq_b_top {
        ...
        assert!(
            (seq_a_score - seq_b_score).abs() > 0.01,
...
fn test_mistral_comprehensive_state_management() {
    ...
    for (name, starter_tokens) in conversation_starters {
        let mut state = model
            .make_state()
            .unwrap_or_else(|_| panic!("Should create state for {}", name));
    ...
    for step in 0..3 {
        ...
        for conversation in &mut conversations {
            ...
            let output = model
                .predict_with_state(&[&token_tensor, &mask], &mut conversation.state)
    ...
    assert!(
        unique_sequences >= 2,
        "Should generate diverse conversation paths"
    );
    ...
    for (pos, &token) in base_context.iter().enumerate() {
        ...
        let mask = create_causal_mask(pos);
        let _output = model
            .predict_with_state(&[&token_tensor, &mask], &mut base_state)
    ...
    for &branch_token in &branch_tokens {
        ...
        for (pos, &token) in base_context.iter().enumerate() {
            ...
            let mask = create_causal_mask(pos);
            let _output = model
                .predict_with_state(&[&token_tensor, &mask], &mut branch_state)
        ...
        let output = model
            .predict_with_state(&[&branch_tensor, &mask], &mut branch_state)
    ...
    for i in 0..10 {
        let state = model
            .make_state()
            .unwrap_or_else(|_| panic!("Should create state {}", i));

        // Use the state briefly
        let token_tensor =
            Tensor::from_vec(vec![1i64], (1, 1), &device).expect("Create test token");
    ...
    for (pos, &token) in context_a.iter().enumerate() {
        ...
        let mask = create_causal_mask(pos);
        let _output = model
            .predict_with_state(&[&token_tensor, &mask], &mut state_a)
    ...
    for (pos, &token) in context_b.iter().enumerate() {
        ...
        let mask = create_causal_mask(pos);
        let _output = model
            .predict_with_state(&[&token_tensor, &mask], &mut state_b)
            .expect("Build context B");
    }

    // Test same input token with both states
    let test_token = Tensor::from_vec(vec![50i64], (1, 1), &device).expect("Create test token");

    let mask_a = create_causal_mask(context_a.len());
    let mask_b = create_causal_mask(context_b.len());

    let output_a = model
        .predict_with_state(&[&test_token, &mask_a], &mut state_a)
        .expect("Test with state A");
    let output_b = model
        .predict_with_state(&[&test_token, &mask_b], &mut state_b)
    ...
    println!(
        "  State A (context {:?} + 50) → Token {} (score: {:.3})",
        context_a, top_a, score_a
    );
    println!(
        "  State B (context {:?} + 50) → Token {} (score: {:.3})",
        context_b, top_b, score_b
    );
    ...
    assert!(
        distribution_diff > 0.001,
        "Different contexts should affect predictions (avg diff: {:.6})",
        distribution_diff
    );
    ...
    if let Some(model_path) = mistral_path {
        ...
        match CoreMLModel::load_from_file(&model_path, &config) {
            Ok(model) => {
                ...
                if let Ok(mut state) = model.make_state() {
                    let input_tensor =
                        Tensor::from_vec(vec![1i64], (1, 1), &device).expect("Create input tensor");
                    ...
                    match model.predict_with_state(&[&input_tensor, &causal_mask], &mut state) {
                        Ok(output) => {
                            let logits =
                                output.to_vec3::<f32>().expect("Extract logits")[0][0].clone();
                            ...
                            println!("    Output shape: {:?}", output.shape());
                            println!(
                                "    Logits stats: min={:.3}, max={:.3}, mean={:.3}, std={:.3}",
                            ...
                            let diversity = indexed_logits
                                .iter()
                                .take(100)
                                .map(|(_, score)| (*score * 1000.0) as i32)
                            ...
                            assert!(
                                top_confidence > -20.0,
                                "Mistral should have reasonable confidence"
                            );
                ...
                Ok(embeddings_model) => {
                    ...
                    match embeddings_model.forward(&[&test_token]) {
                        Ok(hidden_states) => {
                            ...
                            let max_val =
                                hidden_flat.iter().fold(f32::NEG_INFINITY, |a, &b| a.max(b));
                            let mean_val =
                                hidden_flat.iter().sum::<f32>() / hidden_flat.len() as f32;
                            let std_val = {
                                let variance = hidden_flat
                                    .iter()
                                    .map(|&x| (x - mean_val).powi(2))
                                    .sum::<f32>()
                                    / hidden_flat.len() as f32;
                                variance.sqrt()
                            };
                            ...
                            let non_zero_count =
                                hidden_flat.iter().filter(|&&x| x.abs() > 1e-6).count();
                            let zero_ratio =
                                1.0 - (non_zero_count as f32 / hidden_flat.len() as f32);

                            println!(
                                "      🔍 Zero analysis: {}/{} values are zero ({:.1}%)",
                                hidden_flat.len() - non_zero_count,
                                hidden_flat.len(),
                                zero_ratio * 100.0
                            );
                            ...
                            if zero_ratio > 0.99 {
                                println!(
                                    "    🚨 CRITICAL ISSUE: Embeddings output is mostly zeros!"
                                );
                                ...
                                println!(
                                    "    ⚠️  WARNING: Embeddings output has many zeros ({:.1}%)",
                                    zero_ratio * 100.0
                                );
                            } else {
        ...
        if ffn_path.exists() {
            ...
            match CoreMLModel::load_from_file(&ffn_path, &ffn_config) {
                Ok(ffn_model) => {
                    ...
                    let hidden_dim = 1024;
                    let test_hidden = Tensor::ones((1, 1, hidden_dim), DType::F32, &device)
                        .expect("Create test hidden states");
                    let position_ids =
                        Tensor::from_vec(vec![0i64], (1,), &device).expect("Create position_ids");
                    let current_pos =
                        Tensor::from_vec(vec![0i64], (1,), &device).expect("Create current_pos");

                    let mask_data = vec![0.0f32; 512]; // Allow all positions
                    let causal_mask = Tensor::from_vec(mask_data, (1, 1, 1, 512), &device)
                        .expect("Create causal mask");

                    if let Ok(mut ffn_state) = ffn_model.make_state() {
                        match ffn_model.predict_with_state(
                            &[&test_hidden, &position_ids, &current_pos, &causal_mask],
                            &mut ffn_state,
                        ) {
                            Ok(ffn_output) => {
                                let output_vec =
                                    ffn_output.to_vec3::<f32>().expect("Extract FFN output");
                                let output_flat = &output_vec[0][0];

                                let min_val =
                                    output_flat.iter().fold(f32::INFINITY, |a, &b| a.min(b));
                                let max_val =
                                    output_flat.iter().fold(f32::NEG_INFINITY, |a, &b| a.max(b));
                                let mean_val =
                                    output_flat.iter().sum::<f32>() / output_flat.len() as f32;
                                ...
                                let non_zero_count =
                                    output_flat.iter().filter(|&&x| x.abs() > 1e-6).count();
                                let zero_ratio =
                                    1.0 - (non_zero_count as f32 / output_flat.len() as f32);

                                println!(
                                    "      🔍 Zero analysis: {}/{} values are zero ({:.1}%)",
                                    output_flat.len() - non_zero_count,
                                    output_flat.len(),
                                    zero_ratio * 100.0
                                );

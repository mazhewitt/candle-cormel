...
impl QualityTestCase {
    ...
    fn expect_contains_word(mut self, word: &str) -> Self {
        self.semantic_expectations
            .push(SemanticExpectation::ShouldContainWord(word.to_string()));
        self
    }

    fn expect_coherent(mut self) -> Self {
        self.semantic_expectations
            .push(SemanticExpectation::ShouldBeCoherent);
        self
    }

    fn expect_logical_completion(mut self) -> Self {
        self.semantic_expectations
            .push(SemanticExpectation::ShouldCompleteLogically);
        self
    }

    fn expect_token_in_vocab(mut self) -> Self {
        self.semantic_expectations
            .push(SemanticExpectation::TokenShouldBeInVocab);
        self
    }
...
impl ModelQualityVerifier {
        ...
        for test_case in test_cases {
            ...
            if let Some(expected_token) = test_case.exact_token_expectation {
                if result.generated_token != expected_token {
                    regression_count += 1;
                    println!(
                        "   ⚠️ REGRESSION: Expected token {}, got {}",
                        expected_token, result.generated_token
                    );
                }
            }
            ...
            println!(
                "   🎯 Generated: Token {} -> '{}'",
                result.generated_token, result.decoded_output
            );
    ...
    fn run_single_test(&mut self, test_case: &QualityTestCase) -> Result<TestResult> {
        ...
        for expectation in &test_case.semantic_expectations {
            let passes = match expectation {
                SemanticExpectation::ShouldContainWord(word) => decoded_output.contains(word),
                SemanticExpectation::ShouldNotContainWord(word) => !decoded_output.contains(word),
                SemanticExpectation::ShouldBeCoherent => self.assess_coherence(&decoded_output),
    ...
    fn create_test_cases(&self) -> Vec<QualityTestCase> {
        vec![
            ...
            QualityTestCase::new("The capital of France is", "Basic factual knowledge")
                .expect_contains_word("Paris")
                .expect_token_in_vocab()
                .expect_coherent(),
            // Language understanding tests
            QualityTestCase::new("Hello, how are", "Conversational completion")
                .expect_token_in_vocab()
                .expect_coherent()
                .expect_logical_completion(),
            // Pattern completion tests
            QualityTestCase::new("1, 2, 3, 4,", "Numerical sequence")
                .expect_token_in_vocab()
                .expect_coherent(),
            // Common sense tests
            QualityTestCase::new("The sky is", "Common knowledge completion")
                .expect_token_in_vocab()
                .expect_coherent()
                .expect_logical_completion(),
        ]
...
async fn test_comprehensive_model_quality() -> Result<()> {
    ...
    println!(
        ...
        results.overall_quality_score * 100.0
    );
    println!(
        "Regression Detected: {}",
    ...
    assert!(
        results.overall_quality_score >= 0.6,
        "Model quality score too low: {:.1}%",
        results.overall_quality_score * 100.0
    );

    // At least basic coherence should work
    let coherence_tests = results
        .test_results
        .values()
        .filter(|r| {
            r.semantic_analysis.iter().any(|(exp, passed)| {
                matches!(exp, SemanticExpectation::ShouldBeCoherent) && *passed
            })
        })
        .count();
    ...
    assert!(
        coherence_tests > 0,
        "No tests passed basic coherence checks"
    );
...
async fn test_output_consistency() -> Result<()> {
    ...
    assert!(
        all_same,
        "Model outputs are not consistent across runs: {tokens:?}"
    );
...
fn test_verifier_creation() {
    ...
    let test_cases = [QualityTestCase::new("test", "Test case").expect_coherent()];

...
async fn test_qwen_model_download_and_load() -> Result<()> {
    ...
    assert!(true, "Model loaded successfully");
...
async fn test_qwen_tokenization() -> Result<()> {
    ...
    assert!(tokens.len() <= TEST_SEQUENCE_LENGTH, "Tokens should fit in sequence length");
    
    // Verify tokens are in valid range
    for &token in &tokens {
        assert!(token >= 0, "Token IDs should be non-negative");
        assert!((token as usize) < QWEN_VOCAB_SIZE, "Token IDs should be within vocabulary");
    }
...
async fn test_qwen_stateless_inference() -> Result<()> {
    ...
    assert_eq!(output_shape.dims()[1], QWEN_VOCAB_SIZE, "Output should match vocab size");
    ...
    assert_eq!(output_data[0].len(), QWEN_VOCAB_SIZE, "Should have vocab size outputs");
...
async fn test_qwen_stateful_inference() -> Result<()> {
    ...
    let output = setup.model.predict_with_state(&[&input_tensor], &mut state)?;
...
async fn test_qwen_streaming_generation() -> Result<()> {
    ...
    for step in 0..3 {
        ...
        assert!((next_token as usize) < QWEN_VOCAB_SIZE, "Generated token should be within vocabulary");
    }
    
    // Verify we generated new tokens
    assert!(generated_tokens.len() > initial_tokens.len(), "Should have generated new tokens");
...
async fn test_qwen_performance_baseline() -> Result<()> {
    ...
    assert!(inference_time.as_millis() < 1000, 
        "Inference should complete within 1 second, took: {:?}", inference_time);

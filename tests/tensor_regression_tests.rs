...
fn test_generic_model_config_parsing() {
    ...
    let model_config: ModelConfig =
        serde_json::from_str(&config_json).expect("Should be able to parse generic model config");

    // Validate the config matches our captured data
    assert_eq!(
        model_config.shapes.hidden_size,
        captured.embeddings.shape[2]
    );
...
fn test_sampling_with_captured_logits() {
    ...
    let greedy_token =
        sampling::greedy_sample(&logits.squeeze(0).unwrap().squeeze(0).unwrap()).unwrap();
    assert_eq!(greedy_token, captured.generated_token);

    // Test top-k sampling with k=1 (should also match)
    let topk_token =
        sampling::sample_top_k(&logits.squeeze(0).unwrap().squeeze(0).unwrap(), 1, 0.0).unwrap();
    assert_eq!(topk_token, captured.generated_token);

    // Test that top-k with higher k returns valid token
    let topk_10 =
        sampling::sample_top_k(&logits.squeeze(0).unwrap().squeeze(0).unwrap(), 10, 0.7).unwrap();

...
fn test_with_cpu_only_option(
    ...
    use candle_coreml::conversion::{create_multi_feature_provider, tensor_to_mlmultiarray, extract_output};
    ...
    autoreleasepool(|_| -> Result<(), Box<dyn std::error::Error>> {
        ...
        let prediction = unsafe { 
            model.inner_model().predictionFromFeatures_options_error(protocol_provider, &options)

...
use candle_coreml::{CoreMLModelBuilder, Config};
use std::fs;
...
fn create_mock_model_structure() -> tempfile::TempDir {
    ...
    fs::write(model_path.join("metadata.json"), r#"{"model_type": "CoreML"}"#).unwrap();
...
fn test_builder_new_creation() {
    ...
    assert_eq!(builder.config().max_sequence_length, config.max_sequence_length);
...
fn test_load_from_hub_invalid_model_id() {
    ...
    let result = CoreMLModelBuilder::load_from_hub(
        "invalid/model/id/with/too/many/slashes",
        None,
        None,
    );
    ...
    if let Err(error) = result {
        ...
        assert!(error_msg.contains("Failed to create HF API") || error_msg.contains("Failed to get"));
    }
}

...
impl SchemaExtractor {
    ...
    pub fn extract_inputs(&self, manifest: &Value) -> Result<HashMap<String, TensorConfig>> {
        ...
        } else if manifest.get("itemInfoEntries").and_then(|v| v.as_object()).is_some() {
    ...
    pub fn extract_outputs(&self, manifest: &Value) -> Result<HashMap<String, TensorConfig>> {
        ...
        if let Some(output_schema) = manifest.get(0).and_then(|m| m.get("outputSchema").and_then(|s| s.as_array())) {
            debug!("📖 Parsing output schema with {} outputs", output_schema.len());
        ...
        } else if let Some(functions) = manifest.get(0).and_then(|m| m.get("functions").and_then(|f| f.as_array())) {
    ...
    fn try_extract_function_inputs(&self, manifest: &Value) -> Result<Option<HashMap<String, TensorConfig>>> {
            ...
            for prefer in ["prefill", "infer"] {
                for function in funcs {
                    if let Some(func_name) = function.get("name").and_then(|n| n.as_str()) {
                        if func_name == prefer {
                            if let Some(input_schema) = function.get("inputSchema").and_then(|s| s.as_array()) {
                                debug!("📖 Using {} function input schema with {} inputs", prefer, input_schema.len());
    ...
    fn backfill_from_function_schemas(&self, manifest: &Value, outputs: &mut HashMap<String, TensorConfig>) -> Result<()> {
        let functions = manifest.get(0).and_then(|m| m.get("functions").and_then(|f| f.as_array()));
    ...
    fn merge_function_outputs(&self, func_output_schema: &[Value], outputs: &mut HashMap<String, TensorConfig>) -> Result<()> {
    ...
    fn extract_outputs_from_functions(&self, functions: &[Value]) -> Result<HashMap<String, TensorConfig>> {
        let mut outputs = HashMap::new();

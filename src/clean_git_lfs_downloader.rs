...
fn clone_hf_repo_git2(config: &CleanDownloadConfig) -> Result<PathBuf> {
    ...
    let _repo = builder.clone(&repo_url, &config.target_dir)
...
fn scan_for_lfs_pointers(repo_path: &Path, verbose: bool) -> Result<Vec<LfsPointer>> {
    ...
    if verbose {
        ...
        for pointer in &lfs_pointers {
            println!("    • {} (size: {} bytes)", 
                pointer.file_path.strip_prefix(repo_path).unwrap_or(&pointer.file_path).display(),
                pointer.size
            );
        }
    }
...
fn scan_directory_for_lfs(
    ...
    for entry in entries {
        ...
        if path.is_dir() {
            ...
            scan_directory_for_lfs(&path, repo_root, lfs_pointers, verbose)?;
...
fn check_lfs_pointer_file(file_path: &Path, _repo_root: &Path) -> Result<LfsPointer> {
    ...
    let mut file = fs::File::open(file_path)
        .map_err(|e| E::msg(format!("Failed to open file {}: {}", file_path.display(), e)))?;
    
    let mut buffer = [0; 1024];
    let bytes_read = file.read(&mut buffer)
        .map_err(|e| E::msg(format!("Failed to read file {}: {}", file_path.display(), e)))?;
...
fn parse_lfs_pointer(content: &str, file_path: PathBuf) -> Result<LfsPointer> {
    ...
    for line in &lines[1..] {
        if line.starts_with("oid sha256:") {
            oid = line[11..].to_string(); // Skip "oid sha256:"
...
fn download_lfs_content(
    ...
    if verbose {
        println!("📥 Downloading {} LFS files using hf-hub...", lfs_pointers.len());
    }
    ...
    for (i, pointer) in lfs_pointers.iter().enumerate() {
        if verbose {
            println!("  📥 [{}/{}] Downloading: {}", 
                i + 1, 
                lfs_pointers.len(), 
                pointer.file_path.file_name().unwrap_or_default().to_string_lossy()
            );
        }
        ...
        let relative_path = pointer.file_path.strip_prefix(repo_root)
        ...
        match repo.get(&relative_path_str) {
            Ok(downloaded_path) => {
                ...
                if verbose {
                    println!("    ✅ Downloaded and replaced: {} ({} bytes)", 
            ...
            Err(e) => {
                ...
                return Err(E::msg(format!("Failed to download LFS file {}: {}", relative_path_str, e)));
            }
        }
    }
...
pub fn verify_download_completeness(
    ...
    for expected_file in expected_files {
        ...
        if is_lfs_pointer_file(&file_path)? {
            return Err(E::msg(format!("File {} is still an LFS pointer", expected_file)));
        }
...
mod tests {
    ...
    fn test_config_creation() {
        ...
        assert!(config.target_dir.to_string_lossy().contains("clean-test--model"));

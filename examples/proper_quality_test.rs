...
fn main() -> Result<()> {
    ...
    let test_cases = [
        ...
        ("Complete this sentence: The sky is", "Should be sensible completion"),
        ("Hello, how are you", "Should be conversational"),
        ("1, 2, 3, 4,", "Should continue sequence logically"),
        ("Explain AI in simple terms:", "Should give coherent explanation"),
    ];
            ...
            Ok(tokens) => {
                ...
                if let Ok(decoded) = model.tokenizer().decode(&tokens_u32, false) {
                    ...
                    let is_relevant = match i {
                        0 => decoded.to_lowercase().contains("paris") || 
                             decoded.to_lowercase().contains("france"),
                        1 => decoded.to_lowercase().contains("blue") || 
                             decoded.to_lowercase().contains("clear") ||
                             decoded.len() > 10,
                        2 => decoded.to_lowercase().contains("fine") || 
                             decoded.to_lowercase().contains("good") ||
                             decoded.to_lowercase().contains("you"),
                        3 => decoded.contains("5") || 
                             decoded.chars().any(|c| c.is_numeric()),
                    ...
                    if quality_passed {
                        ...
                        println!("   ❌ FAIL: Coherent={}, Relevant={}", is_coherent, is_relevant);
                    }
    ...
    println!("Quality Score: {:.1}% ({}/{} tests passed)", quality_score, passed, total);
    ...
    println!("  Multi-token approach:  {:.1}% (proper completion testing)", quality_score);

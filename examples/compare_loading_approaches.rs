use candle_coreml::{QwenConfig, QwenModel, UnifiedModelLoader, ensure_model_downloaded};
use anyhow::Result;
...
fn main() -> Result<()> {
    ...
    println!("Unified config batch size: {:?}", unified_config.components.iter().find_map(|(name, c)| {
    ...
    println!("Manual config batch size: {:?}", manual_config.model_config.components.iter().find_map(|(name, c)| {
        if name.contains("embeddings") {
            c.inputs.get("input_ids").map(|t| &t.shape)
        } else {
            None

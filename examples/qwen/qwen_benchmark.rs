...
impl BenchmarkResult {
    ...
    fn stddev_latency(&self) -> f64 {
        ...
        let variance = self.latency_ms
            .iter()
            .map(|&x| (x - mean).powi(2))
            .sum::<f64>() / (self.latency_ms.len() - 1) as f64;
        variance.sqrt()
    }
...
impl QwenBenchmark {
    ...
    fn generate_test_tokens(&self, sequence_length: usize) -> Result<Vec<i64>> {
        ...
        let test_text = "The quick brown fox jumps over the lazy dog. ".repeat(sequence_length / 10 + 1);
    ...
    fn benchmark_inference(
        ...
        let device_type = if use_state { "ANE (Stateful)" } else { "CPU (Stateless)" };
        let mut result = BenchmarkResult::new(device_type.to_string(), sequence_length);
        
        if verbose {
            println!("🔧 Benchmarking {} with sequence length {}", device_type, sequence_length);
        }
        ...
        if use_state {
            ...
            let _ = self.model.predict_with_state(&[&input_tensor], &mut state)?;
        } else {
...
fn print_results(results: &[BenchmarkResult]) {
    ...
    for (&seq_len, group) in results_by_length.iter() {
        ...
        for result in group {
            ...
            println!("   • Mean latency:    {:.2}ms ± {:.2}ms", 
                result.mean_latency(), result.stddev_latency());
            println!("   • Min/Max latency: {:.2}ms / {:.2}ms", 
                result.min_latency(), result.max_latency());
            println!("   • Throughput:      {:.1} tokens/second", 
...
fn run_benchmark(args: &Args) -> Result<()> {
    ...
    let sequence_lengths = args.sequence_lengths
        .as_ref()
        .map(|v| v.as_slice())

...
fn main() -> Result<()> {
        ...
        match model.generate_tokens_topk_temp(prompt, 25, 0.7, Some(50)) {
            Ok(tokens) => {
                ...
                if let Ok(decoded) = model.tokenizer().decode(&tokens_u32, false) {
                    ...
                    let thinking_indicators = ["think", "Thinking", "reason", "consider", "because"];
    ...
    let thinking_tokens = ["Thinking", "thinking", "Think", "think", "reason", "because", "step"];
    for token_text in &thinking_tokens {
        ...
        if let Ok(encoded) = model.tokenizer().encode(*token_text, false) {
            if !encoded.get_ids().is_empty() {
                println!("   ✅ '{}' → token IDs: {:?}", token_text, encoded.get_ids());
            }
        } else {
            println!("   ❌ '{}' not found in vocabulary", token_text);
        }
    }

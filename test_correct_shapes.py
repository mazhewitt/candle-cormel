#!/usr/bin/env python3
"""
test_correct_shapes.py
Test with correct shapes - FFN expects single token processing
"""
...
def test_correct_shapes():
    ...
    try:
        ...
        # FFN expects single token processing: (1, 1, 1024)
        hidden_dim = 1024
        
        # Create hidden states for single token
        hidden_states = np.random.randn(1, 1, hidden_dim).astype(np.float32)
        
        # Create causal mask for single token - should be (1, 1)
        causal_mask = np.zeros((1, 1), dtype=np.float32)
        
        # Position for current token
        position_ids = np.array([0], dtype=np.int32)  # Current position
        
        try:
            print(f"\n🧪 Testing FFN with single token shapes:")
            ...
            print(f"✅ SUCCESS with single token processing!")
